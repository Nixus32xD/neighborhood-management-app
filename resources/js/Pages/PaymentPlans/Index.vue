...
const form = useForm({ unit_id: '', owner_id: '', items: [], installments_count: 1, start_date: new Date().toISOString().slice(0, 10), notes: '' })
  ...
  <DashboardLayout title="Planes de Pago">
    <Card title="Planes de pago" subtitle="Financiación de deuda existente sin alterar las expensas originales">
      ...
      <div class="overflow-x-auto rounded-lg border border-slate-200">
        <table class="w-full text-sm"><thead class="bg-slate-50"><tr><th class="p-3 text-left">UF</th><th class="p-3 text-left">Propietario</th><th class="p-3 text-right">Original</th><th class="p-3 text-right">Abonado</th><th class="p-3 text-right">Saldo</th><th class="p-3 text-center">Cuotas</th><th class="p-3 text-left">Próximo vencimiento</th><th class="p-3 text-left">Estado</th><th class="p-3"></th></tr></thead>
          <tbody class="divide-y"><tr v-for="plan in plans" :key="plan.id"><td class="p-3 font-medium">{{ plan.uf_number }}</td><td class="p-3">{{ plan.owner }}</td><td class="p-3 text-right">{{ money(plan.original_amount) }}</td><td class="p-3 text-right">{{ money(plan.paid_amount) }}</td><td class="p-3 text-right font-semibold">{{ money(plan.outstanding_amount) }}</td><td class="p-3 text-center">{{ plan.installments_paid }}/{{ plan.installments_count }}</td><td class="p-3">{{ plan.next_installment?.due_date || '-' }}</td><td class="p-3">{{ statusLabel(plan.status) }}</td><td class="p-3"><Link :href="route('payment-plans.show', plan.id)" class="text-emerald-700 hover:underline">Ver detalle</Link></td></tr><tr v-if="!plans?.length"><td colspan="9" class="p-8 text-center text-slate-500">No hay planes para el filtro seleccionado.</td></tr></tbody>
        </table>
      </div>
    </Card>
    <Modal :show="showCreate" title="Nuevo Plan de Pago" max-width="2xl" @close="showCreate = false">
      <form class="space-y-4" @submit.prevent="submit">
        ...
        <p v-if="selectedUnit" class="text-right font-semibold">Total seleccionado: {{ money(total) }}</p>
        ...
        <p v-if="total && form.installments_count" class="text-sm text-slate-600">Cuota estimada: {{ money(total / Number(form.installments_count)) }}; el ajuste de centavos se aplica en la última cuota.</p>
      ...
      <template #footer><div class="flex justify-end gap-2"><Button variant="secondary" @click="showCreate = false">Cancelar</Button><Button :disabled="!selectedUnit || total <= 0" :loading="form.processing" @click="submit">Crear plan</Button></div></template>
    </Modal>
  </DashboardLayout>
</template>
